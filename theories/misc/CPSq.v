...
Definition M A := forall Ω : Prop, (A -> Ω) -> Ω.
Definition ret {A} (x : A) : M A := fun Ω k => k x.

(** The underlying type is the type of stacks *)
Inductive Typeᴿ : M (Prop -> Type) -> Type :=
| IsType : forall (A : Prop -> Type),
  ((forall Ω : Prop, A Ω -> Ω) -> Type) -> Typeᴿ (ret A)
.

Definition TYPE := sig (M (Prop -> Type)) Typeᴿ.

Definition Typeᶠ : TYPE :=
 exist _ _ _ (IsType (fun Ω : Prop => (Prop -> Type) -> Ω) Typeᴿ).

Definition C (Ω : Prop) (A : TYPE) : Type :=
  match A.(prf) with IsType A Aᴿ => A Ω end.

Definition Cᴿ (A : TYPE) : (forall Ω : Prop, C Ω A -> Ω) -> Type :=
  match A.(prf) as T return
    (forall Ω, match T with IsType A _ => _ end -> _) -> Type
  with IsType A Aᴿ => Aᴿ end.

Definition El (A : TYPE) : Type := sig (forall Ω : Prop, C Ω A -> Ω) (Cᴿ A).
